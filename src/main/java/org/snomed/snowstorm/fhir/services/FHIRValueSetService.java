...
public class FHIRValueSetService implements FHIRConstants {
	...
	public ValueSet expand(final ValueSetExpansionParameters params, String displayLanguage) {
		...
		// the caller never typed. Built per path because the paths do not filter alike, and a
		// reporter that does not match its own path's filter either invents a match reason or
		...
		if (isSnomed && allInclusionVersions.size() > 1) {
			...
			// This path filters in memory on a lowercased substring, the same rule the inline
			// path uses, so it reports matches the same way. Not the indexed predicate: no
			// Elasticsearch query ran here.
