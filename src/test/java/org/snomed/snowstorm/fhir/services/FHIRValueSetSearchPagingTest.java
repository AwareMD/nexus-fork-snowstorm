...
class FHIRValueSetSearchPagingTest extends AbstractFHIRTest {
	...
	/**
	 * What the index held before this class stored anything. Other FHIR tests share the index and
	 * some leave value sets behind, so every total below is asserted as growth over this baseline
	 * rather than as an absolute; an absolute made the class pass or fail on test ordering.
	 */
	private long baseline;

	@BeforeEach
	void storeValueSets() {
		baseline = valueSetRepository.count();
	...
	void testCountIsHonoured() {
		...
		assertEquals(baseline + STORED_VALUE_SETS + 2, bundle.getTotal());
	...
	void testEveryStoredValueSetIsReachableByFollowingNext() {
		...
		assertEquals(baseline + STORED_VALUE_SETS + 2, total, "Reported total");
		assertEquals(baseline + STORED_VALUE_SETS + 2, seen.size(), "Value sets actually reachable by paging");
		assertTrue(seen.containsAll(storedIds), "Every value set this class stored was reached");
